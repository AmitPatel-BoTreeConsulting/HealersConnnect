class Website::CoursesController < ApplicationController
  ...
  def index
    @courses = Course.find_all_by_course_category_id(CourseCategory.first.id).sort_by &:name
    @course = @courses.first
  end

  def category_wise_courses
    respond_to do |format|
      @courses = Course.find_all_by_course_category_id(params[:id]).sort_by &:name
